    ...
    try {
        ...
        promotionPayload.PromosRequested.forEach(promo => {
            if (promo.ID==='ext-promo') {
                const discountAmount = Promotion50OffMens(promo.ID,promotionPayload)
                promotionResponse.PromosAccepted.push(
                    {
                        ID: promo.ID,                        
...
function Promotion50OffMens(promoID: string, promotionPayload: PromotionIntegrationPayload): number {
    ...
    console.log(`Order.ID=${promotionPayload.Order.ID} | promotionPayloadpromo.ID=${promoID} | discountAmount=${discountAmount}`);
